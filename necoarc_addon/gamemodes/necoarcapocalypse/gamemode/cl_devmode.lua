...
--  Исправленная версия: увеличенные шрифты, рабочие кнопки,
--  корректная сетка апгрейдов, спавн врагов и пр.
-- ============================================================
...
--  ШРИФТЫ (увеличенные)
-- ============================================================
surface.CreateFont("NAA_Dev_Title", { font="Trebuchet MS", size=32, weight=900, antialias=true })
surface.CreateFont("NAA_Dev_H2",    { font="Trebuchet MS", size=22, weight=700, antialias=true })
surface.CreateFont("NAA_Dev_Body",  { font="Trebuchet MS", size=18, weight=500, antialias=true })
surface.CreateFont("NAA_Dev_Small", { font="Trebuchet MS", size=15, weight=600, antialias=true })
surface.CreateFont("NAA_Dev_Mono",  { font="Courier New", size=14, weight=400, antialias=true })
surface.CreateFont("NAA_Dev_Icon",  { font="Trebuchet MS", size=26, weight=900, antialias=true })
...
-- ============================================================
--  СОЗДАНИЕ ПАНЕЛИ (увеличенные размеры)
-- ============================================================
local DevPanel  = nil
local PW, PH    = 1280, 780
local TAB_W     = 200
...
local function BuildPlayerTab(parent)
    ...
    for _, cls in ipairs({"survivor", "medic", "berserker", "hunter"}) do
        ...
        btn:SetPos(clsX, 68); btn:SetSize(148, 38); btn:SetText("")
        local cid = cls
    ...
    local function MakeSlider(labelText, ypos, min_, max_, startVal, cb)
        ...
        lbPanel:SetPos(16, ypos); lbPanel:SetSize(CONT_W - 32, 40)
        ...
        apply.DoClick = function() cb(math.floor(sl:GetValue())) end
        return sl
    end

    local slHP    = MakeSlider("HP:", 118, 1, 9999, 100, function(v) SendDev("set_hp", {hp=v}) end)
    local slArmor = MakeSlider("Броня:", 162, 0, 500, 100, function(v) SendDev("set_armor", {armor=v}) end)
    local slSpeed = MakeSlider("Скорость:", 206, 50, 3000, 200, function(v) SendDev("set_speed", {speed=v}) end)
    local slLives = MakeSlider("Жизни:", 250, 0, 999, 5, function(v) SendDev("set_lives", {lives=v}) end)
    ...
    for _, bd in ipairs(BTNS) do
        local bdata = bd
        local btn   = vgui.Create("DButton", p)
        btn:SetPos(bx, BTN_Y); btn:SetSize(190, 40); btn:SetText("")
        btn.Paint = function(s, w, h)
    ...
    statPanel:SetPos(CONT_W - 280, 48); statPanel:SetSize(270, 280)
    statPanel.Paint = function(s, w, h)
        ...
        local lines = {
            ...
            {"Стаки охотника:", tostring(NAA.HunterStacks or 0), C.accent1},
        }
        ...
        if NAA_DEV.GodMode then
            draw.RoundedBox(4, 8, h-36, w-16, 28, Color(120, 20, 50, 200))
            draw.SimpleText("GOD MODE ACTIVE", "NAA_Dev_Small", w/2, h-22,
...
-- ============================================================
--  ТАБ: АПГРЕЙДЫ (увеличенные карточки, 4 колонки)
-- ============================================================
local function BuildUpgradesTab(parent)
    ...
    for _, cat in ipairs(CATS) do
        local cdat = cat
        local btn  = vgui.Create("DButton", p)
        btn:SetPos(fx, 8); btn:SetSize(130, 32); btn:SetText("")
        ...
        btn.DoClick = function()
            ...
            if IsValid(p.Layout) then
                p.Layout:InvalidateLayout(true)
            end
        end
        fx = fx + 136
    end
    ...
    local massX = fx + 10
    local massActions = {
        {label="МАКС ВСЕ",    cmd="give_all_upgrades", args={}},
        {label="СБРОСИТЬ",     cmd="reset_upgrades",    args={}},
    }
    for _, ma in ipairs(massActions) do
        local mdat = ma
        local btn  = vgui.Create("DButton", p)
        btn:SetPos(massX, 8); btn:SetSize(150, 32); btn:SetText("")
        btn.Paint = function(s, w, h)
    ...
    scroll:SetPos(4, 46); scroll:SetSize(CONT_W - 8, CONT_H - 54)
    ...
    layout:SetPos(0, 4)
    layout:SetSize(CONT_W - 14, CONT_H - 60)
    layout:SetSpaceX(6); layout:SetSpaceY(6)
    p.Layout = layout

    local CARD_W, CARD_H = 220, 140
    p.UpgradeRows = {}
    ...
    for _, entry in ipairs(upgList) do
        ...
        card.Paint = function(s, w, h)
            ...
            -- Иконка категории
            draw.SimpleText(cat.icon or "•", "NAA_Dev_Icon", 14, 26, cat.color, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            -- Название
            draw.SimpleText(upg.name, "NAA_Dev_Small", w/2 + 8, 14, Color(240,235,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            -- Редкость
            draw.SimpleText(rc.name, "NAA_Dev_Small", w-6, 6, rc.color, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
            -- Описание (обрезаем)
            local shortDesc = upg.desc
            if #shortDesc > 35 then shortDesc = shortDesc:sub(1,32) .. "..." end
            draw.SimpleText(shortDesc, "NAA_Dev_Small", 4, 28, C.text_dim, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
            ...
            draw.SimpleText(curStacks .. "/" .. maxStacks, "NAA_Dev_Small", w/2, h - 11,
                curStacks >= maxStacks and C.gold or C.text_dim, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        ...
        btnMinus:SetPos(4, CARD_H - 38); btnMinus:SetSize(32, 24); btnMinus:SetText("-")
        ...
        btnPlus:SetPos(38, CARD_H - 38); btnPlus:SetSize(32, 24); btnPlus:SetText("+")
        ...
        btnMax:SetPos(72, CARD_H - 38); btnMax:SetSize(60, 24); btnMax:SetText("МАКС")
...
-- ============================================================
--  ТАБ: ВОЛНЫ (без изменений, но оставляем)
-- ============================================================
local function BuildWavesTab(parent)
    ...
    p.Paint = function(s, w, h)
        draw.RoundedBox(6, 12, 44, 280, 90, Color(8,5,18))
        surface.SetDrawColor(C.sep); surface.DrawOutlinedRect(12,44,280,90)
        draw.SimpleText("Текущая волна:", "NAA_Dev_Body", 24, 64, C.text_dim)
        draw.SimpleText(tostring(NAA and NAA.CurrentWave or 0), "NAA_Dev_Title", 200, 78, C.gold, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText("Враги живые:", "NAA_Dev_Body", 24, 98, C.text_dim)
        draw.SimpleText(tostring(NAA and NAA.AliveEnemies or 0), "NAA_Dev_H2", 200, 98, C.red, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        draw.SimpleText("Фаза:", "NAA_Dev_Body", 24, 118, C.text_dim)
        draw.SimpleText(tostring(NAA and NAA.Phase or "?"), "NAA_Dev_Body", 200, 118, C.accent2, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end

    local waveLbl = vgui.Create("DLabel", p)
    waveLbl:SetPos(16, 150); waveLbl:SetSize(160,20)
    ...
    waveEntry:SetPos(16, 174); waveEntry:SetSize(100, 32)
    ...
    setWaveBtn:SetPos(120, 174); setWaveBtn:SetSize(100,32); setWaveBtn:SetText("ПЕРЕЙТИ")
    ...
    for _, qb in ipairs(quickBtns) do
        local qdat = qb
        local btn  = vgui.Create("DButton", p)
        btn:SetPos(16, quickY); btn:SetSize(260, 40); btn:SetText("")
        btn.Paint = function(s,w,h)
            draw.RoundedBox(5,0,0,w,h, s:IsHovered() and C.tab_hov or Color(12,8,22))
            surface.SetDrawColor(C.sep); surface.DrawOutlinedRect(0,0,w,h)
            draw.SimpleText(qdat.label, "NAA_Dev_Body", w/2, h/2, C.text, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
    ...
    diffLbl:SetPos(320, 44); diffLbl:SetSize(200,20)
    ...
    for did, diff in pairs(NAA.Difficulties) do
        local ddat = diff
        local btn  = vgui.Create("DButton", p)
        btn:SetPos(320, dy); btn:SetSize(220, 36); btn:SetText("")
        btn.Paint = function(s,w,h)
            local isActive = (NAA and NAA.Difficulty == ddat.id)
            draw.RoundedBox(4,0,0,w,h, isActive and Color(ddat.color.r, ddat.color.g, ddat.color.b, 150) or (s:IsHovered() and C.tab_hov or Color(12,8,22)))
            surface.SetDrawColor(ddat.color); surface.DrawOutlinedRect(0,0,w,h)
            draw.SimpleText(ddat.name, "NAA_Dev_Body", w/2, h/2, C.text, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
        btn.DoClick = function() SendDev("start_game", {difficulty=ddat.id}) end
        dy = dy + 42
    end
    return p
end
...
local function BuildEconomyTab(parent)
    ...
    p:SetPos(0,0); p:SetSize(CONT_W, CONT_H)
    p.Paint = function(s,w,h)
        draw.RoundedBox(6,12,44,300,100, Color(8,5,18))
        surface.SetDrawColor(C.sep); surface.DrawOutlinedRect(12,44,300,100)
        draw.SimpleText("Монеты:", "NAA_Dev_Body", 24, 64, C.text_dim)
        draw.SimpleText(tostring(NAA.ClientCoins or 0), "NAA_Dev_Title", 200, 84, C.gold, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    local lbl = vgui.Create("DLabel", p)
    lbl:SetPos(16,12); lbl:SetSize(300,24)
    ...
    coinLbl:SetPos(16,160); coinLbl:SetSize(200,20)
    ...
    coinEntry:SetPos(16,184); coinEntry:SetSize(120,34)
    ...
    setBtn:SetPos(142,184); setBtn:SetSize(100,34); setBtn:SetText("УСТАНОВИТЬ")
    ...
    addLbl:SetPos(16,240); addLbl:SetSize(200,20)
    ...
    for _, amount in ipairs(addAmounts) do
        local amt = amount
        local btn = vgui.Create("DButton", p)
        btn:SetPos(ax, 264); btn:SetSize(90, 36); btn:SetText("+"..amt)
    ...
    neoLbl:SetPos(16,320); neoLbl:SetSize(300,20)
    ...
    for _, amount in ipairs(neoAmounts) do
        local amt = amount
        local btn = vgui.Create("DButton", p)
        btn:SetPos(nx, 344); btn:SetSize(100, 36); btn:SetText("+"..amt)
...
-- ============================================================
--  ТАБ: СИНЕРГИИ (исправлен скролл и отступы)
-- ============================================================
    ...
    for _, syn in ipairs(NAA.Synergies) do
        ...
        card.Paint = function(s,w,h)
            ...
            if active then
                ...
                draw.SimpleText("✔ АКТИВНА", "NAA_Dev_Small", w-8, 10, col, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
            else
                draw.SimpleText("○ неактивна", "NAA_Dev_Small", w-8, 10, C.text_dim, TEXT_ALIGN_RIGHT, TEXT_ALIGN_TOP)
            end
            draw.SimpleText(syndat.name, "NAA_Dev_H2", 16, 14, active and col or C.text_dim)
            draw.SimpleText(syndat.desc, "NAA_Dev_Body", 16, 40, active and C.text or C.text_dim)
            local reqStr = "Требует: " .. table.concat(syndat.requires, " + ")
            if syndat.classRequired then reqStr = reqStr .. "  |  Класс: " .. syndat.classRequired end
            draw.SimpleText(reqStr, "NAA_Dev_Small", 16, 68, C.text_dim)
        end
        sy = sy + 96
    end
...
-- ============================================================
--  ТАБ: СПАВН (исправлен: вызывает консольную команду)
-- ============================================================
local function BuildSpawnTab(parent)
    ...
    countSlider:SetPos(120,40); countSlider:SetSize(250,30)
    ...
    for _, edata in ipairs(ENEMY_LIST) do
        local ed = edata
        local btn = vgui.Create("DButton", p)
        btn:SetPos(ex, ey); btn:SetSize(210, 48); btn:SetText("")
        btn.Paint = function(s,w,h)
            draw.RoundedBox(5,0,0,w,h, s:IsHovered() and Color(ed.col.r/5, ed.col.g/5, ed.col.b/5, 220) or Color(10,7,20))
            surface.SetDrawColor(ed.col); surface.DrawOutlinedRect(0,0,w,h)
            draw.SimpleText(ed.name:upper(), "NAA_Dev_Small", w/2, h/2 - 4, ed.col, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            draw.SimpleText("нажми для спавна", "NAA_Dev_Small", w/2, h/2 + 12, C.text_dim, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
        btn.DoClick = function()
            local count = math.floor(countSlider:GetValue())
            RunConsoleCommand("naa_spawnenemy", ed.id, tostring(count))
        end
        colCount = colCount + 1
        if colCount >= 4 then
            colCount = 0
    ...
    wepLbl:SetPos(16, ey + 70); wepLbl:SetSize(300,20)
    ...
    for _, wd in ipairs(WEAPONS) do
        local wdat = wd
        local btn  = vgui.Create("DButton", p)
        btn:SetPos(wx, ey + 94); btn:SetSize(130, 36); btn:SetText(wdat.label)
...
function NAA_CreateDevPanel()
    ...
    closeBtn:SetPos(PW-40, 8); closeBtn:SetSize(30, 26)
    closeBtn:SetText("✕"); closeBtn:SetFont("NAA_Dev_H2"); closeBtn:SetTextColor(C.text_dim)
    closeBtn.Paint = function(s,w,h)
        if s:IsHovered() then draw.RoundedBox(4,0,0,w,h, Color(80,15,25)); s:SetTextColor(C.red)
        else s:SetTextColor(C.text_dim) end
    end
    ...
    for i, tab in ipairs(TABS) do
        ...
        btn:SetPos(2, 44 + (i-1)*56); btn:SetSize(TAB_W-2, 52)
        btn:SetText("")
        btn.Paint = function(s,w,h)
            local isActive = activeTab == tdat.id
            local hovered  = s:IsHovered()
            local bg = isActive and C.tab_act or (hovered and C.tab_hov or C.tab_bg)
            ...
            local col = isActive and Color(255,255,255) or (hovered and Color(200,190,240) or C.text_dim)
            draw.SimpleText(tdat.icon, "NAA_Dev_Icon", 22, h/2-6, col, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            draw.SimpleText(tdat.label, "NAA_Dev_Small", w/2+12, h/2+8, col, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        end
    ...
    timer.Create("NAA_DevSynUpdate", 0.5, 0, function()
        if not IsValid(f) then timer.Remove("NAA_DevSynUpdate") return end
...
hook.Add("KeyPress", "NAA_DevKey", function(ply, key)
    if key == KEY_F9 then ToggleDevPanel() end
end)

-- ============================================================
--  NET: ПОЛУЧЕНИЕ СТАТУСА СЕРВЕРА (исправлено: net.ReadTable)
-- ============================================================
net.Receive("NAA_DevStatus", function()
    ...
    if data.diff    then NAA.ClientDifficulty = data.diff end
    if data.lives   then NAA.ClientLives  = data.lives   end
    if data.coins   then NAA.ClientCoins  = data.coins   end
end)
...
hook.Add("HUDPaint", "NAA_DevHint", function()
    ...
    draw.RoundedBox(4, 8, ScrH()-36, 200, 26, Color(6,3,15,180))
    surface.SetDrawColor(C.accent1); surface.DrawOutlinedRect(8, ScrH()-36, 200, 26)
    draw.SimpleText("DEV [F9]", "NAA_Dev_Small", 18, ScrH()-24, C.accent1, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    if NAA_DEV.GodMode then
        draw.RoundedBox(4, 216, ScrH()-36, 130, 26, Color(80,10,30,200))
        surface.SetDrawColor(C.red); surface.DrawOutlinedRect(216, ScrH()-36, 130, 26)
        draw.SimpleText("GOD MODE", "NAA_Dev_Small", 226, ScrH()-24, C.red)
    end
end)

-- ============================================================
--  LIGHT STEP (оставлен как есть)
-- ============================================================
hook.Add("Think", "NAA_LightStepClient", function()
    ...
    if punch:Length() > 0.01 then
        ply:ViewPunch(Angle(-punch.p * (1 - factor), -punch.y * (1 - factor) * 0.5, 0))
    end
end)
...
hook.Add("PostDrawOpaqueRenderables", "NAA_DetectiveHPBars", function()
    ...
    for _, ent in ipairs(ents.GetAll()) do
        ...
        draw.SimpleText(hp .. "/" .. maxhp, "NAA_Dev_Small", x, y - barH - 4, Color(255,255,255,220), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
    end
end)
...
print("[NAA] cl_devmode.lua загружен (исправленная версия)")