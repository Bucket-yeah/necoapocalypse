...
if SERVER then
    ...
    function ENT:ApplyPassiveAura()
        ...
        for _, ent in ipairs(ents.FindInSphere(self:GetPos(), HEAL_RADIUS)) do
            if IsValid(ent) and ent.IsNecoArc and ent ~= self and ent:Health() > 0 and ent:Health() < ent:GetMaxHealth() then
    ...
    function ENT:OnKilled(dmg)
        ...
        -- Вызываем глобальную функцию учёта убийств
        if NAA_OnNecoKilled then
            NAA_OnNecoKilled(self, attacker)
        end

        -- Вызываем родительский метод, чтобы DrGBase корректно завершил процессы (рэгдолл и т.д.)
        self.BaseClass.OnKilled(self, dmg)

        -- Применяем бафф к союзникам через таймер, чтобы избежать конфликтов с удалением энтити
