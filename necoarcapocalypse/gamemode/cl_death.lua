...
function NAA_ShowDeath()
    ...
    local retry = vgui.Create("DButton", f)
    retry:SetPos(cx, sh*0.82) retry:SetSize(btnW, btnH)
    retry:SetText(" Попробовать снова") retry:SetFont("NAA_Med")
    retry:SetTextColor(Color(255,255,255))
    retry.Paint = function(s,w,h)
        draw.RoundedBox(8,0,0,w,h, s:IsHovered() and Color(140,20,20) or Color(100,15,15))
    end
    ...
    menuBtn:SetPos(cx, sh*0.82+58) menuBtn:SetSize(btnW, btnH)
    menuBtn:SetText(" Главное меню") menuBtn:SetFont("NAA_Med")
