-- ============================================================
--  NECO ARC APOCALYPSE — init.lua (SERVER)
--  Исправлено: добавлена отправка фазы новому игроку
-- ============================================================
DeriveGamemode("base")
...
-- ============================================================
--  GM:PlayerInitialSpawn — отправляем фазу новому игроку
-- ============================================================
...
-- ============================================================
--  СБРОС СЧЁТЧИКОВ ПРИ СТАРТЕ ВОЛНЫ
-- ============================================================
...
function NAA.GameOver()
    ...
    timer.Simple(15, function() NAA.ResetGame() end)
end
...
net.Receive("NAA_SelectClass", function(len, ply)
    ...
    NAA.ReadyPlayers[ply:SteamID()] = "class"

    local allReady = true
    for _, p in player.Iterator() do
        if IsValid(p) and NAA.ReadyPlayers[p:SteamID()] ~= "class" then
...
net.Receive("NAA_Ready", function(len, ply)
    NAA.ReadyPlayers[ply:SteamID()] = "ready"
end)
...
function GM:PlayerSpawn(ply)
    ...
    ply:StripWeapons()
    for _, wep in ipairs(cls.startWeapons or {}) do
        ply:Give(wep)
    end
...
function GM:PlayerDeath(ply, inflictor, attacker)
        ...
        timer.Simple(0.1, function()
            if IsValid(ply) then
                ...
                if (pd.synergies or {}).immortal_berserk and pd.class == "berserker" then
                    ...
                    net.Start("NAA_SpecialAlert")
                        net.WriteString("⚡ БЕССМЕРТНЫЙ БЕРСЕРК! ×2.8 урона НАВСЕГДА!")
                    net.Send(ply)
                end
                net.Start("NAA_SpecialAlert")
    ...
    if pd.lives <= 0 then
        ...
        for _, p in player.Iterator() do
            if IsValid(p) and not GetPD(p).dead then
                anyAlive = true; break
            end
        end
        ...
        timer.Simple(5, function()
            if IsValid(ply) and NAA.Phase == NAA.PHASE_WAVE then
                ply:Spawn()
            end
        end)
    end
...
function GM:PlayerDisconnected(ply)
    if IsValid(ply) then
        ...
        for _, ent in ipairs(ents.GetAll()) do
            if IsValid(ent) and ent.NecoOwner == ply then
                ent:Remove()
            end
        end
    end
...
concommand.Add("naa_debug", function(ply, cmd, args)
...
concommand.Add("naa_skipwave", function(ply, cmd, args)
    if IsValid(ply) and not ply:IsSuperAdmin() then return end
    if not GetConVar("sv_cheats"):GetBool() then
        if IsValid(ply) then ply:ChatPrint("[NAA] Requires sv_cheats 1") end
        return
    end
    for _, ent in ipairs(ents.GetAll()) do
...
concommand.Add("naa_godmode", function(ply, cmd, args)
    if not IsValid(ply) then return end
    if not GetConVar("sv_cheats"):GetBool() then
        ply:ChatPrint("[NAA] Requires sv_cheats 1"); return
    end
    local pd = GetPD(ply)
    pd.devGodMode = not pd.devGodMode
    if pd.devGodMode then
...
concommand.Add("naa_setwave", function(ply, cmd, args)
    if IsValid(ply) and not ply:IsSuperAdmin() then return end
    if not GetConVar("sv_cheats"):GetBool() then
        if IsValid(ply) then ply:ChatPrint("[NAA] Requires sv_cheats 1") end; return
    end
...
concommand.Add("naa_status", function(ply, cmd, args)
    ...
    for _, p in player.Iterator() do
        if IsValid(p) then
            local pd = GetPD(p)
            print(string.format("  %s | class=%s lives=%d kills=%d coins=%d hunters=%d shots=%d",
                p:Nick(), pd.class or "?", pd.lives or 0, pd.kills or 0,
                pd.coins or 0, pd.hunterStacks or 0,
                NAA.ShotCounter[p:SteamID()] or 0
            ))
        end
    end
end)
...
hook.Add("PostGamemodeLoaded", "NAA_Init", function()
    ...
    print("[NAA] DevMode: нажми F9 при sv_cheats 1")
end)